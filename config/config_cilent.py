...
# 크롤링 병렬화 설정 (main.py)
# 키워드 병렬 + 소스 병렬 + 소스별 동시요청 상한(세마포어)의 3중 구조.
#
# 병렬화의 진짜 위험은 "같은 사이트에 동시에 꽂히는 요청 수"다(=차단 유발).
# 그래서 키워드/소스를 아무리 병렬로 돌려도, 소스별 세마포어로 같은 사이트에
# 동시에 진입하는 크롤러 수를 아래 상한으로 묶는다.
#   - API 소스(tavily/youtube): IP 차단이 아니라 쿼터 방식이라 동시 요청에 관대 → 넉넉히
#   - 커뮤니티 스크래퍼(namuwiki/natepann/dcinside): 차단이 공격적 → 낮게
# 아직 임베딩/실측 전 초기값이며, 차단이 관찰되면 해당 소스 값을 낮춘다.
KEYWORD_WORKERS = 4         # 동시에 처리할 키워드 수
SOURCE_CONCURRENCY = {      # 소스별 동시 크롤 상한 (전체 키워드 스레드를 통틀어)
    "tavily":   4,
    "youtube":  4,
    "namuwiki": 2,
    "natepann": 2,
