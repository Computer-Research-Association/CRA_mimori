...
- 사용맥락: 쓰이는 상황·대상·감정이 드러나면 2점이다.
           ※ 형식이 '뜻 설명'이더라도 그 안에 **누가 · 언제 · 왜** 쓰는지가 담겨 있으면
             사용맥락에 답한 것으로 보고 2점을 준다.
             (예: "내또출은 '내일 또 출근'의 줄임말로, 월요일 출근 스트레스를 표현한다"
              → 직장인이 출근을 앞두고 쓴다는 상황이 드러나므로 2점)
             상황·대상 없이 정의만 있으면 1점이다.
...
- 실제로 쓰인 모습이 보이지만 그것만으로는 상황을 알기 어려운 경우
...
점수: 0
인용: "없음"
이유: 표현이 반복되지만 감상뿐이고 언제·어떻게 쓰는지에 대한 설명이 전혀 없다.
