...
# 하루 늦게 반영됨). 1로 낮춰 실측값에 맞췄었다.
#
# 그런데 8/19 실측은 (수동 테스트라) 낮 시간대에 한 것이었고, 8/7에 스케줄러 실행
# 시각이 14:00→02:00로 바뀐 뒤로는 안 맞았다. 실측(2026-09-14 02:21 KST 배치 직후):
# 서로 무관한 키워드 34/40개가 동일하게 마지막 날짜(전날)에 ratio=0.0으로 찍힘 →
# 같은 시각 10:03 KST에 동일 키워드를 재조회하니 그 날짜에 정상적인 실측값(예:
# 84.2, 63.5, 67.1)이 들어와 있었다 — 즉 새벽 2시엔 데이터랩이 전날 데이터를 아직
# 발행하지 않아 응답에서 생략됐을 뿐인데, 이를 "진짜 0"으로 채워 넣어 z가 대거
# -8.00 근처로 폭락(대량 "소멸")했다. 배치가 새벽에 도는 한 1일 지연은 불충분하므로
# 2로 되돌린다. (트렌드가 최대 하루 더 과거를 보게 되는 대가보다, 매일 새벽 대량
# 오탐이 나는 쪽이 훨씬 나쁘다.)
PUBLISH_LAG_DAYS = 2
