...
#
# 2026-09-12: NAVER_MIN_IQR 을 2.0→1.0으로 낮춤. 저빈도 신조어는 실제 IQR이
# 0에 가까워 이 바닥값이 분모를 그대로 지배하는데, 2.0에서는 today-median 차이가
# 4 이상 나야 "핫함"(z>2) 문턱을 넘어 대부분 "평상"에 뭉개지는 현상이 있었다
# (trend_scores 실측 없이 코드 분석만으로 내린 잠정 조정 — 값을 반으로 낮춰
# 민감도를 2배로 키우되 바닥값 자체는 유지해 분모 0에 의한 z 폭주는 계속 막는다).
# 너무 낮추면 예전에 min_iqr 도입으로 잡았던 저빈도 노이즈 폭주가 재발할 수 있으니,
# 실제 trend_scores 분포(diagnose_zscore.py)로 검증되면 이 값도 재조정할 것.
NAVER_MIN_IQR = 1.0
GOOGLE_MIN_IQR = 2.0
KAKAO_MIN_IQR = 5.0
