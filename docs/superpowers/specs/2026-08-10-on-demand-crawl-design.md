...
**알려진 제약 (구현 완료 후 최종 리뷰에서 발견, 의도적으로 미해결)**: 온디맨드로 수집된 키워드는 트렌드 점수가 절대 안 붙는다 — 워커가 `crawl_all()`만 호출하고, 매일 밤 자동 크롤이 하는 `judge_and_report()`(트렌드 판정+저장) 호출은 하지 않기 때문. `crawlers/Keywords.md`에도 추가 안 되므로 다음날 야간 배치로도 못 채워짐. `/api/trend/<keyword>`는 이 경우 404를, `/api/analyze`·`/api/rag`는 `trend: null`을 정상적으로 반환하도록 이미 방어돼 있어 에러는 안 나지만, 온디맨드 키워드는 영구히 트렌드 정보 없이 남는다. 필요해지면 워커에 `judge_and_report()` 호출을 추가하거나 `Keywords.md`에 자동 등재하는 후속 작업으로 해결.
